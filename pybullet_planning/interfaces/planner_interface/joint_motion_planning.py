...
def plan_joint_motion(body, joints, end_conf, obstacles=[], attachments=[],
    ...
    if not check_initial_end(start_conf, end_conf, collision_fn, diagnosis=diagnosis):
        return None
    return birrt(start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn, **kwargs)#调用的 rrt_connect 主意最大次数从20改为5000了

    #return plan_lazy_prm(start_conf, end_conf, sample_fn, extend_fn, collision_fn)#测试可用

    # return rrt_connect(start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn,
    #             max_iterations=5000)#算法代码已经解析和测试

    # return rrt_star(start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn, radius=1,
    #         goal_probability=.2, informed=False, verbose=False, draw_fn=None)#需要改写
